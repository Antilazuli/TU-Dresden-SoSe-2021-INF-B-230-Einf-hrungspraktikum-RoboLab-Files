...
class Robot_Control:
    ...
    def sensorScanForPaths(self):
        self.motorsRawMovement(self.ticPerDeg * 360)
        while "running" in self.motorA.state:
            if self.motorA.position < self.ticPerDeg * 360:
                if self.motorA.position > self.ticPerDeg * 45 and self.motorA.position < self.ticPerDeg * 135:
                    self.pathScan[1] = True
                elif self.motorA.position > self.ticPerDeg * 135 and self.motorA.position < self.ticPerDeg * 225:
                    self.pathScan[2] = True
                elif self.motorA.position > self.ticPerDeg * 225 and self.motorA.position < self.ticPerDeg * 315:
